/*
 * Enable or disable debug code. Initialize thread's registers and stack to known values (useful to know how much space
 * of the stack is used by the thread).
 *
 * Notes:
 ...
 *
 * Note:
 *   1. It is recommenced to set highest priority adjusted to the application to minimize memory used by the scheduler.
 */
...
/*
 * Enable or disable round-robin scheduler. If enabled, each tick will try to switch thread context to another ready to
 * run thread of the same priority.
 *
 * Notes:
...
/*
 * Add possibility to override NVIC_PRIO_BITS if CMSIS is not used and platform have less or more than default (4 bits).
 *
 * Notes:
 ...
 * priority ISR. Application should not call any nOS API from interrupt service routines with priority higher than this
 * setting. Can't be set to zero.
 *
