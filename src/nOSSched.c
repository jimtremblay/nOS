...
extern "C" {
  ...
  #if (NOS_CONFIG_HIGHEST_THREAD_PRIO < 32)
      nOS_AppendToList((nOS_List*)&nOS_readyThreadsList[prio], &thread->readyWait);
  ...
  #if (NOS_CONFIG_HIGHEST_THREAD_PRIO < 32)
      nOS_RemoveFromList((nOS_List*)&nOS_readyThreadsList[prio], &thread->readyWait);
...
#endif
    {
        /* Trap waiting thread in loop until ready to run, necessary to trap main thread
         * when returning from ISR and no thread is allowed to run */
        do {
            /* Check if current running thread is the highest prio thread */
            nOS_highPrioThread = nOS_FindHighPrioThread();
            if (nOS_highPrioThread == NULL) {
                nOS_PeekCritical();
            } else if (nOS_runningThread != nOS_highPrioThread) {
                nOS_SwitchContext();
            }
        } while (nOS_runningThread->state != NOS_THREAD_READY);
        err = NOS_OK;
    }
...
nOS_Error nOS_Init(void)
{
...
#endif
    {
        ...
        for (i = 0; i <= NOS_CONFIG_HIGHEST_THREAD_PRIO; i++) {
            nOS_InitList((nOS_List*)&nOS_readyThreadsList[i]);
        }
#else
        nOS_InitList((nOS_List*)&nOS_readyThreadsList);
#endif
#if (NOS_CONFIG_WAITING_TIMEOUT_ENABLE > 0) || (NOS_CONFIG_SLEEP_ENABLE > 0) || (NOS_CONFIG_SLEEP_UNTIL_ENABLE > 0)
        nOS_InitList((nOS_List*)&nOS_timeoutThreadsList);
#endif
#if (NOS_CONFIG_THREAD_SUSPEND_ALL_ENABLE > 0)
        nOS_InitList((nOS_List*)&nOS_allThreadsList);
#endif
        ...
        nOS_mainHandle.ext = NULL;
#if (NOS_CONFIG_THREAD_NAME_ENABLE > 0)
        nOS_mainHandle.name = "main";
#endif
        nOS_mainHandle.error = (int)NOS_OK;
        nOS_mainHandle.readyWait.payload = (void*)&nOS_mainHandle;
#if (NOS_CONFIG_WAITING_TIMEOUT_ENABLE > 0) || (NOS_CONFIG_SLEEP_ENABLE > 0) || (NOS_CONFIG_SLEEP_UNTIL_ENABLE > 0)
        nOS_mainHandle.tout.payload = (void*)&nOS_mainHandle;
        nOS_mainHandle.timeout = 0;
#endif
#if (NOS_CONFIG_THREAD_SUSPEND_ALL_ENABLE > 0)
        nOS_mainHandle.node.payload = (void*)&nOS_mainHandle;
        nOS_AppendToList((nOS_List*)&nOS_allThreadsList, (nOS_Node*)&nOS_mainHandle.node);
#endif
        nOS_AppendThreadToReadyList((nOS_Thread*)&nOS_mainHandle);
...
nOS_Error nOS_Yield(void)
{
...
#endif
    {
        nOS_EnterCritical(sr);
#if (NOS_CONFIG_HIGHEST_THREAD_PRIO > 0)
        nOS_RotateList((nOS_List*)&nOS_readyThreadsList[nOS_runningThread->prio]);
#else
        nOS_RotateList((nOS_List*)&nOS_readyThreadsList);
#endif
        err = nOS_Schedule();
        nOS_LeaveCritical(sr);
    }
...
void nOS_Tick(nOS_TickCounter ticks)
{
...
#endif
    {
        nOS_EnterCritical(sr);
#if (NOS_CONFIG_WAITING_TIMEOUT_ENABLE > 0) || (NOS_CONFIG_SLEEP_ENABLE > 0) || (NOS_CONFIG_SLEEP_UNTIL_ENABLE > 0)
        nOS_WalkInList((nOS_List*)&nOS_timeoutThreadsList, nOS_TickThread, &ticks);
#endif
